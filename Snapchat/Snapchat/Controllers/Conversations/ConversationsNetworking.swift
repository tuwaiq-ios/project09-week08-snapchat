...
class ConversationsNetworking {
    ...
    func observeFriendsList() {
        ...
        Database.database().reference().child("friendsList").child(User.id).observeSingleEvent(of: .value) { (snap) in
...
private func observeFriendActions(){
    ...
    private func observeRemovedFriends(){
        Database.database().reference().child("friendsList").child(User.id).observe(.childRemoved) { (snap) in
    ...
    private func observeNewFriends(){
        Database.database().reference().child("friendsList").child(User.id).observe(.childAdded) { (snap) in
    ...
    private func messagesReference(){
        for key in friendKeys {
            Database.database().reference().child("messages").child(User.id).child(key).queryLimited(toLast: 1).observeSingleEvent(of: .value) { (snap) in
    ...
    func observeNewMessages(completion: @escaping (_ newMessages: [Messages]) -> Void){
        for key in friendKeys {
            Database.database().reference().child("messages").child(User.id).child(key).queryLimited(toLast: 1).observe(.childAdded) { (snap) in
    ...
    func observeDeletedMessages() {
        for key in friendKeys {
            Database.database().reference().child("messages").child(User.id).child(key).queryLimited(toLast: 1).observe(.childRemoved) { (snap) in
                guard let values = snap.value as? [String: Any] else { return }
                let message = ChatKit.setupUserMessage(for: values)
    ...
    func observeUserSeenMessage(_ friendId: String, completion: @escaping(_ userSeenMessagesCount: Int) -> Void){
        let ref = Database.database().reference().child("messages").child("unread-Messages").child(friendId).child(User.id)
        ref.observe(.value) { (snap) in
    ...
    func observeIsUserTyping(_ friendId: String, completion: @escaping (_ isTyping: Bool, _ friendId: String) -> Void){
        let ref = Database.database().reference().child("userActions").child(friendId).child(User.id)
        ref.observe(.value) { (snap) in
    ...
    func observeUnreadMessages(_ key: String, completion: @escaping(_ unreadMessages: [String: Int]) -> Void){
        ...
        Database.database().reference().child("messages").child("unread-Messages").child(User.id).child(key).observe(.value) { (snap) in
            self.totalUnread = 0
        ...
        Database.database().reference().child("messages").child("unread-Messages").child(User.id).child(key).observe(.childRemoved) { (snap) in
