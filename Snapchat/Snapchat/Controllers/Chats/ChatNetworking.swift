...
class ChatNetworking {
    ...
    func getMessages(_ v: UIView, _ m: [Messages], completion: @escaping(_ newMessages: [Messages], _ mOrder: Bool) -> Void){
        ...
        if firstMessage == nil{
            nodeRef = Database.database().reference().child("messages").child(User.id).child(friend.id ?? "").queryOrderedByKey().queryLimited(toLast: messageCount)
            messageOrder = true
        }else{
            let mId = firstMessage!.id
            nodeRef = Database.database().reference().child("messages").child(User.id).child(friend.id ?? "").queryOrderedByKey().queryEnding(atValue: mId).queryLimited(toLast: messageCount)
            messageOrder = false
        }
    ...
    private func sendMediaMessage(url: String, _ image: UIImage, _ id: String){
        ...
        let values = ["sender": User.id!, "time": Date().timeIntervalSince1970, "recipient": friend.id!, "mediaUrl": url, "width": image.size.width, "height": image.size.height, "messageId": messageId, "storageID": id] as [String: Any]
        senderRef.updateChildValues(values)
        friendRef.updateChildValues(values)
        let unreadRef = Database.database().reference().child("messages").child("unread-Messages").child(friend.id ?? "").child(User.id).child(senderRef.key!)
    ...
    func observeIsUserTyping(completion: @escaping (_ friendActivity: FriendActivity) -> Void){
        readMessagesHandler()
        let db = Database.database().reference().child("userActions").child(friend.id ?? "").child(User.id)
    ...
    private func readMessagesHandler(){
        let unreadRef = Database.database().reference().child("messages").child("unread-Messages").child(User.id).child(friend.id ?? "")
    ...
    func removeObserves(){
        Database.database().reference().child("messages").child("unread-Messages").child(User.id).child(friend.id ?? "").removeAllObservers()
    }
    
    
    func observeUserMessageSeen() {
        Database.database().reference().child("messages").child("unread-Messages").child(friend.id ?? "").child(User.id).observe(.value) { (snap) in
