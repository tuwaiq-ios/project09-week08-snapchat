...
class ChatViewController: UIViewController ,UIImagePickerControllerDelegate, UINavigationControllerDelegate, AVAudioRecorderDelegate{
    ...
    private func setupTextMessage(){
        ...
        let senderRef = Database.database().reference().child("messages").child(User.id).child(friendId).childByAutoId()
        let friendRef = Database.database().reference().child("messages").child(friendId).child(User.id).child(senderRef.key!)
        guard let messageId = senderRef.key else { return }
        var values = ["message": trimmedMessage, "sender": User.id!, "recipient": friend.id!, "time": Date().timeIntervalSince1970, "messageId": messageId] as [String : Any]
    ...
    private func observeMessageActions(){
        ...
        let ref = Database.database().reference().child("messages").child(User.id).child(friend.id ?? "")
        ...
        ref.queryLimited(toLast: 1).observe(.childAdded) { (snap) in
            self.chatNetworking.newMessageRecievedHandler(self.messages, for: snap) { (newMessage) in
                ...
                if newMessage.determineUser() != User.id {
