...
class ChatViewController: UIViewController ,UIImagePickerControllerDelegate, UINavigationControllerDelegate, AVAudioRecorderDelegate{
    ...
    private func setupTextMessage(){
        ...
        let senderRef = Database.database().reference().child("messages").child(userId).child(friendId).childByAutoId()
        let friendRef = Database.database().reference().child("messages").child(friendId).child(userId).child(senderRef.key!)
        guard let messageId = senderRef.key else { return }
        var values = ["message": trimmedMessage, "sender": userId, "recipient": friend.id!, "time": Date().timeIntervalSince1970, "messageId": messageId] as [String : Any]
    ...
    private func observeMessageActions(){
        ...
        guard let userId = Auth.auth().currentUser?.uid else {return}
        let ref = Database.database().reference().child("messages").child(userId).child(friend.id ?? "")
